<x-layouts.admin-layout title="Edit Katalog">

    <div class="max-w-2xl mx-auto bg-white p-6 rounded-xl shadow-xl">
        <h2 class="text-2xl font-bold mb-6">Edit Katalog</h2>

        <form action="{{ route('admin.katalog.update', $katalog->catalogue_id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="4"
                    class="mt-1 block w-full rounded-md border-stone-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 sm:text-sm">{{ old('description', $katalog->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="status_publish" id="status_publish"
                    class="mt-1 block w-full rounded-md border-stone-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 sm:text-sm">
                    <option value="Y"
                        {{ old('status_publish', $katalog->status_publish) == 'Y' ? 'selected' : '' }}>Ya</option>
                    <option value="N"
                        {{ old('status_publish', $katalog->status_publish) == 'N' ? 'selected' : '' }}>Tidak</option>
                </select>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="image" id="image" value="{{ old('image', $katalog->image) }}"
                    class="mt-1 block w-full rounded-md border-stone-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 sm:text-sm">
                @error('image')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="flex justify-end space-x-2">
                ...
                <button type="submit"
                    class="px-4 py-2 bg-amber-700 text-white rounded-lg hover:bg-amber-800">Update</button>
            </div>
